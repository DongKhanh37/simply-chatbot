...
export async function generateIntro(fileNames) {
    ...
    return "Xin chào! Đây là chatbot thông tin về 2 hãng xe Tesla và VinFast. Tôi có thể giúp gì cho bạn hôm nay?";
}
...
export async function fetchLocalAIResponse(prompt) {
    if (knowledgeBase.length === 0) {
        return "Xin lỗi, hệ thống chưa nạp xong tài liệu từ thư mục knowledge/.";
    }
...
QUY TẮC QUAN TRỌNG:
1. Nếu câu hỏi của người dùng chỉ là lời chào hỏi thông thường (ví dụ: xin chào, hi, hello...), hãy chào lại lịch sự và KHÔNG trích dẫn tài liệu.
2. Với các câu hỏi khác, bạn CHỈ ĐƯỢC PHÉP sử dụng thông tin từ TÀI LIỆU SAU ĐÂY để trả lời. Không dùng kiến thức ngoài.
3. Nếu tài liệu không có thông tin để trả lời, hãy nói chính xác là: "Xin lỗi, tôi không tìm thấy thông tin này."
4. LUÔN LUÔN trả lời bằng Tiếng Việt. Kể cả khi tài liệu cung cấp bằng Tiếng Anh, bạn phải dịch và trả lời bằng Tiếng Việt.
...
CÂU HỎI CỦA NGƯỜI DÙNG: ${prompt}`;
    ...
    try {
        ...
        return 'Xin lỗi, đã có lỗi kết nối với OpenAI API. Vui lòng kiểm tra lại kết nối mạng hoặc API Key.';
    }
}
